...
public class Player : MonoBehaviour
{
    ...
    [Header("Recipes")]
    [SerializeField] private GameObject Receta1;
    [SerializeField] private GameObject Receta2;
    [SerializeField] private GameObject Receta3;
    [SerializeField] private GameObject Receta4;
    [SerializeField] private GameObject Receta5;
    [SerializeField] private GameObject Receta6;
    [SerializeField] private GameObject Receta7;
    [SerializeField] private GameObject Receta8;
        ...
        switch (colisio.collider.tag)
        {
            case "mesa":  //encimera
                ...
                if (!enc.hasItem()) { // Deixem carrega.
                    ...
                    if(carry.tag == "Olla" && enc.isPlateReady() && !carry.GetComponent<Olla>().isFull() && !carry.GetComponent<Olla>().isBurned())
                    ...
                    else if (enc.getItemTag() == "Plate" && carry.GetComponent<Ingredient>().isPlateReady() && !enc.getItem2check().GetComponent<Plate>().isLocked())
                    {
                        GameObject item = enc.getItem();
                ...
                else if (isContainer(carry.tag) && isContainer(enc.getItem2check().tag))
                {
                    ...
                    else if (carry.tag == "Plate" && !carry.GetComponent<Plate>().isFull() && !carry.GetComponent<Plate>().isLocked() 
            ...
            case "fogon":
                ...
                else if (fog.hasItem() && isIngredient(carry.tag) && fog.hasContainerSpace() && isCompatible(fog.getItem2check(),carry) && !fog.onFire())
                {
            ...
            case "Horno":
                ...
                else if (carry.tag == "Plate" && carry.GetComponent<Plate>().isEmpty() && !oven.isEmpty() && oven.isDoneCooking())
                {
    ...
    public void setNextCompletedPlate()
    {
        if (GameObject.Find("UIEscena").GetComponent<GestioEscena>().getPedidos().GetComponent<Pedidos>().getPedidosCount() > 0)
        {
            if (carry != null && carry.tag == "Plate")
            {
                carry.GetComponent<Plate>().dumpContent();
                Destroy(carry);
                carry = null;
            ...
            if (carry == null)
            {
                String next = GameObject.Find("UIEscena").GetComponent<GestioEscena>().getPedidos().transform.GetChild(0).tag;
                next = "Receta" + next.Substring(next.Length - 1, 1);
                switch (next)
                {
                    case "Receta1":
                        carry = Instantiate(Receta1,transform.position + new Vector3(0,1f,0),transform.rotation, transform);
                        break;
                    case "Receta2":
                        carry = Instantiate(Receta2, transform.position + new Vector3(0, 1f, 0), transform.rotation, transform);
                        break;
                    case "Receta3":
                        carry = Instantiate(Receta3, transform.position + new Vector3(0, 1f, 0), transform.rotation, transform);
                        break;
                    case "Receta4":
                        carry = Instantiate(Receta4, transform.position + new Vector3(0, 1f, 0), transform.rotation, transform);
                        carry.GetComponent<Plate>().setLocked();
                        break;
                    case "Receta5":
                        carry = Instantiate(Receta5, transform.position + new Vector3(0, 1f, 0), transform.rotation, transform);
                        carry.GetComponent<Plate>().setLocked();
                        break;
                    case "Receta6":
                        carry = Instantiate(Receta6, transform.position + new Vector3(0, 1f, 0), transform.rotation, transform);
                        carry.GetComponent<Plate>().setLocked();
                        break;
                    case "Receta7":
                        carry = Instantiate(Receta7, transform.position + new Vector3(0, 1f, 0), transform.rotation, transform);
                        carry.GetComponent<Plate>().setLocked();
                        break;
                    case "Receta8":
                        carry = Instantiate(Receta8, transform.position + new Vector3(0, 1f, 0), transform.rotation, transform);
                        carry.GetComponent<Plate>().setLocked();
                        break;
                }
                placeInFront();
    ...
    private void pickItem(RaycastHit colisio)
    {
        switch (colisio.collider.tag)
        {
            case "Ingredient":
                ...
                Vector3 itemPos = transform.position + relativePos * 2.5f + new Vector3(0, 1.3f, 0);
            ...
            case "mesa":
                ...
                {
                    carry = colisio.collider.GetComponent<Encimera>().getItem();
                    placeInFront();
                }
                break;
            ...
            case "SpawnPlates":
                //TODO
                if (colisio.collider.GetComponent<PlateSpawner>().hasItem())
                {
                    carry = colisio.collider.GetComponent<PlateSpawner>().getItem();
                    placeInFront();
                }
                break;

            case "fogon":
                if (colisio.collider.GetComponent<Fogon>().hasItem() && !colisio.collider.GetComponent<Fogon>().onFire())
                {
                    carry = colisio.collider.GetComponent<Fogon>().getItem();
                    switch (carry.tag)
                    {
                        case "Plate":
                            carry.GetComponent<Plate>().clearUI();
                            break;
                        case "Olla":
                            carry.GetComponent<Olla>().clearUI();
                            break;
                        case "Sarten":
                            carry.GetComponent<Sarten>().clearUI();
                            break;
                    }
                    placeInFront();
                }
    ...
    {
        float y = carry.transform.position.y;
        Vector3 pos = transform.position + relativePos * 2.5f;
        carry.transform.position = new Vector3(pos.x,y,pos.z);
    }
